				...
				<u-form-item v-for="(item, index) in obj.voteItemlist" :key="index">
					<view style="display: flex; align-items: center; width: 100%">
						<view class="cu-form-group">
							<view class="bg-img" v-for="(option, id) in item.imgList" :key="id" @tap="ViewImage(index)" :data-url="item.imgList[id]">
								<image :src="item.imgList[id]" style="width: 100%; height: 100%" mode="aspectFill"></image>
								<view class="cu-tag bg-red" @tap.stop="DelImg(index)" :data-index="id"><text class="cuIcon-close"></text></view>
							</view>
							<view class="solids" @tap="ChooseImage(index)" v-if="item.imgList.length < 1"><text class="cuIcon-cameraadd" style="font-size: 60rpx"></text></view>
						</view>
			...
			uni.chooseImage({
				...
				success: res => {
					if (this.obj.voteItemlist[index].imgList.length != 0) {
						this.obj.voteItemlist[index].imgList = this.obj.voteItemlist[index].imgList.concat(res.tempFilePaths);
					} else {
			...
			uni.showModal({
				...
				success: res => {
					if (res.confirm) {
						...
						this.obj.voteItemlist[index].imgList.splice(0, 1);
					}
				}
			});
		},
		...
		async uploadingImg() {
			...
			for (let i = 0; i < arrar.length; i++) {
				...
				const res = await uniCloud.uploadFile({
					...
					cloudPath: this.obj.voteItemlist[i].imgList[0]
				});
				if (res) {
